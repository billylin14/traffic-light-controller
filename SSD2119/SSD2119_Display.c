...
static void LCD_GPIOInit(void){
    ...
    // D2
    SYSCTL_RCGCGPIO_R |= 0x1<<3;                // activate port D
    wait++;                                     // wait for port activation
    wait++;                                     // wait for port activation
    GPIO_PORTD_DIR_R |= 0x1<<2;                 // make PD2 an output
    GPIO_PORTD_AFSEL_R &= 0;                    // disable alternate functions
    GPIO_PORTD_DEN_R |= 0x1<<2;                 // enable digital I/O on PD2

    // K5
    SYSCTL_RCGCGPIO_R |= 0x1<<9;                // activate port K
    wait++;                                     // wait for port activation
    wait++;                                     // wait for port activation
    GPIO_PORTK_DIR_R |= 0x1<<5;                 // make PK5 an output
    GPIO_PORTK_AFSEL_R &= 0;                    // disable alternate functions
    GPIO_PORTK_DEN_R |= 0x1<<5;                 // enable digital I/O on PK5

    // M7
    SYSCTL_RCGCGPIO_R |= 0x1<<11;               // activate port M
    wait++;                                     // wait for port activation
    wait++;                                     // wait for port activation
    GPIO_PORTM_DIR_R |= 0x1<<7;                 // make PM7 an output
    GPIO_PORTM_AFSEL_R &= 0;                    // disable alternate functions
    GPIO_PORTM_DEN_R |= 0x1<<7;                 // enable digital I/O on PM7

    // N4, N5
    SYSCTL_RCGCGPIO_R |= 0x1<<12;               // activate port N
    wait++;                                     // wait for port activation
    wait++;                                     // wait for port activation
    GPIO_PORTN_DIR_R |= (0x1<<4)|(0x1<<5);      // make PN4, PN5 outputs
    GPIO_PORTN_AFSEL_R &= 0;                    // disable alternate functions
    GPIO_PORTN_DEN_R |= (0x1<<4)|(0x1<<5);      // enable digital I/O on PN4, PN5

    // P0, P1, P3, P4
    SYSCTL_RCGCGPIO_R |= 0x1<<13;               // activate port P
    wait++;                                     // wait for port activation
    wait++;                                     // wait for port activation
    GPIO_PORTP_DIR_R |= 0x3|(0x3<<3);           // make PP0-1, PP3-4 outputs
    GPIO_PORTP_AFSEL_R &= 0;                    // disable alternate functions
    GPIO_PORTP_DEN_R |= 0x3|(0x3<<3);           // enable digital I/O on PP0-1, PP3-4

    // Q0, Q2, Q3
    SYSCTL_RCGCGPIO_R |= 0x1<<14;               // activate port Q
    wait++;                                     // wait for port activation
    wait++;                                     // wait for port activation
    GPIO_PORTQ_DIR_R |= 0x1|(0x3<<2);           // make PQ0, PQ2-3 outputs
    GPIO_PORTQ_AFSEL_R &= 0;                    // disable alternate functions
    GPIO_PORTQ_DEN_R |= 0x1|(0x3<<2);           // enable digital I/O on PQ0, PQ2-3
}
...
void LCD_PrintFloat(float num){
    ...
    //       lab spec.
    char tempString[64]; //convert doube into string format
    snprintf(tempString, sizeof tempString, "%.2f", num);
    unsigned int i = 0;

    while (i < 5){ //total of 5 chars
        LCD_PrintChar(tempString[i]);
        i++;
    }
}
